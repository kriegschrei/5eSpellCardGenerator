...
parser.add_option('-o','--output',dest='outFile',default='spellCards.pdf',help='PDF file to write to [default=%default]')
...
parser.add_option('-a','--asset-path',dest='assetPath',default='Assets',help='Path to asset directory.  Must have sub-directoriees for SpellsAndClasses and Icons. [default=%default]')
...
SPELL_TITLE_WIDTH         = cardWidthPoints     - FOUR_PCT_WIDTH
SPELL_LEVEL_HEIGHT        = FOUR_PERCENT_HEIGHT + TWELVE_PERCENT_HEIGHT
...
def makeCard(ctx,spell,x,y):
    ...
    addSchoolTextAndReference(ctx,spell.school,spell.reference,x + TEN_PCT_WIDTH,y)
    y += FOUR_PERCENT_HEIGHT
    y += addSpellTitle(
        ctx=ctx,
        x=x,
        y=y,
    ...
    spellLevelBanner(ctx,spell.level,x+SPELL_TITLE_WIDTH-FOURTEEN_PCT_WIDTH-PIXEL_BUFFER,origY-PIXEL_BUFFER)
    ...
    drawText(
        ...
        x = x + SPELL_TITLE_WIDTH - FOURTEEN_PCT_WIDTH,
    ...
    y += PIXEL_BUFFER + addSpellComponents(
        ctx,
        spell,
        SPELL_TITLE_WIDTH,
        x + PIXEL_BUFFER,
    ...
    if spell.materialcomponents:
        ...
        _,h,_ = drawText(
            ...
            targetH = len(spell.materialcomponents.split('\n')) * THREE_PERCENT_HEIGHT,
            talign = 'Center'
        )
        y += h + PIXEL_BUFFER
    # Add a horizontal bar
    y += PIXEL_BUFFER + addRectangle(ctx,x,y,SPELL_TITLE_WIDTH,TWO_PCT_WIDTH,schoolColor)
    # Add the stats ( Casting Time, Range, Target, Duration, Saving Throw, Attack, Effect)
    y += PIXEL_BUFFER + addSpellSpecs(ctx,spell,SPELL_TITLE_WIDTH,x,y)
    # If "AT HIGHER LEVELS"
    # Add the "At Higher Levels" bar and text
    if spell.athigherlevels:
        y += PIXEL_BUFFER + addHigherLevelTitle(
            ctx=ctx,
            x=x,
            y=y,
            width=SPELL_TITLE_WIDTH,
        ...
        w,h,_ = drawText(
            ...
            y = y,
            targetW = SPELL_TITLE_WIDTH,
            targetH = len(spell.athigherlevels.split('\n')) * THREE_PERCENT_HEIGHT,
            threshold=.5,
            color = COLOR_BLACK,
            talign = 'Center',
            halign = 'Center',
    ...
    bottomY -= PIXEL_BUFFER + addClasses(ctx,spell,SPELL_TITLE_WIDTH,x,bottomY)
    # Add a horizontal spacer bar above the classes
    bottomY -= PIXEL_BUFFER + addRectangle(ctx,x,bottomY - TWO_PCT_WIDTH,SPELL_TITLE_WIDTH,TWO_PCT_WIDTH,schoolColor)
    ...
    drawText(
        ...
        x = x + PIXEL_BUFFER,
        y = y,
        targetW = SPELL_TITLE_WIDTH - (PIXEL_BUFFER*2),
        targetH = bottomY - y,
        threshold = 0.1,
        valign = 'Top',
        halign = 'Left',
...
def addClasses(ctx,spell,w,x,y):
    ...
    classIconDiameter = TEN_PCT_WIDTH
    # Collect the font sizes per class
    classFontSizes = {}
    for c in classList:
        cw,ch,fontSize,_ = wrapText(ctx,layout,c.lower(),classTextWidth,THREE_PERCENT_HEIGHT)
        classFontSizes[c] = {
            'w' : cw,
            'fs' : fontSize,
            'h' : ch,
    ...
    y -= fontHeight + classIconDiameter + PIXEL_BUFFER * 2
    ...
    ctx.restore()
    return fontHeight + classIconDiameter + PIXEL_BUFFER
...
def addSpellTitle(ctx,x,y,width,height,color,school,text,fontName=TITLE_FONT,fontColor=COLOR_WHITE,talign='Center'):
    ...
    drawText(
        ...
        text=text,
        fontName=fontName,
        x=x + PIXEL_BUFFER + height + (TWO_PCT_WIDTH/2),
        y=y+(TWO_PCT_WIDTH/2),
        targetW=width - height - FOURTEEN_PCT_WIDTH - TWO_PCT_WIDTH,
...
def addSpellComponents(ctx,spell,w,x,y):
    ...
    for c in componentList:
        cw,ch,fontSize,_ = wrapText(ctx,layout,c.lower(),componentTextWidth,THREE_PERCENT_HEIGHT)
...
def addSchoolTextAndReference(ctx,school,ref,x,y):
    ...
    refX = x + PIXEL_BUFFER
    refY = y + PIXEL_BUFFER
    refH = THREE_PERCENT_HEIGHT
    refW = cardWidthPoints - (TWO_PCT_WIDTH * 2) - (PIXEL_BUFFER * 2) - FOURTEEN_PCT_WIDTH - TEN_PCT_WIDTH
    # Text
